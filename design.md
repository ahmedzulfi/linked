...
- **Landing Page Hero Layout (Website Image Showcase):** To align with premium SaaS product design, the hero is centered and consists of a pill badge, bold headline, description, and pill-shaped CTAs. Below the buttons, a `<HabitlineProductShowcase />` component displays the website preview image (`heroimage.png`) from the public folder. The image container restricts height to clip the bottom half of the image, while a smooth white gradient overlay fades up from the bottom edge to blend the showcase seamlessly into the white page background.
