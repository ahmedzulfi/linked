...
- **Landing Page Hero Layout (Habitline Centered Mockup):** To align with premium SaaS product design, the hero is centered and consists of a pill badge, bold headline, description, and pill-shaped CTAs. Below the buttons, a `<HabitlineProductShowcase />` component displays a central mock mobile screen showing a bento website layout, flanked by floating glassmorphic cards representing database parse metrics and domain mapping, with a smooth white cover gradient fade-out at the bottom.
