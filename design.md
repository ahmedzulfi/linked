...
- **Conversational Builder Workspace:**
  ...
  - **Start Profile Header Card:** Renders a beautiful Instagram-style dark card (`bg-[#0B0F19]`, `rounded-[24px]`) at the very top of the scroll container to introduce the brand's circular avatar, name/role headline, and account handle, with spacing (`mb-8`) to position the initial chat bubble further down.
