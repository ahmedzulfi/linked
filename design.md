...
- **Main Canvas Workspace:**
  ...
  - **Customizable Avatar & Portrait Images:** Substitutes the template placeholders with the user's `avatarUrl` (sidebar and layout avatar) and `bannerUrl` / `avatarUrl` (large hero portrait) in both the published page compiler and client-side editor preview. To enable intuitive image selection in customize mode, all large hero grid images (classes `framer-1u0qqnx`, `framer-1i9qvch`, `framer-nswwcw`) and small heading thumbnail icons (classes `framer-1m8xtt5`, `framer-19db7zm`, `framer-yfhmy0`) are tagged with the corresponding `data-editable-field` fields (`bannerUrl` and `avatarUrl`), ensuring clicking any of the hero images instantly activates their upload editor.
