...
## Layout & Spacing
The layout uses a symmetrical 3-column architecture on the landing page hero and centered layouts elsewhere, maintaining a wide, fixed composition with generous negative space and a soft full-bleed background.
- **Landing Page Hero Layout (Symmetrical Sided-Mobile):** The hero uses a symmetrical 3-column layout on large screens (`grid-cols-1 lg:grid-cols-12`). The left column (`col-span-3`) houses a high-fidelity mobile device mockup displaying a LinkedIn profile layout. The center column (`col-span-6`) contains the centered typographic text stack (badge, display headline, description, and side-by-side CTA buttons). The right column (`col-span-3`) houses a matching mobile device mockup displaying the generated bento-grid website layout. On mobile and tablet screens, the two devices wrap below the center column side-by-side.
