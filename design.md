...
## Layout & Spacing
The layout uses a split-column architecture on the landing page hero and centered layouts elsewhere, maintaining a wide, fixed composition with generous negative space and a soft full-bleed background.
- **Landing Page Hero Layout (Aesthetic Split):** To avoid visual clutter, the hero uses a two-column split layout (`grid-cols-1 lg:grid-cols-12`). The left column (`col-span-7`) houses the text content (badge, display headline, description) and side-by-side CTA buttons. The right column (`col-span-5`) is dedicated to a high-fidelity visual product mockup showing the transformation of a LinkedIn profile into a personal bento site, rather than hosting a functional file input zone.
