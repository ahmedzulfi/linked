...
### Dashboard UI
The dashboard page lists all websites and drafts generated by the user:
- **Websites Grid Container:** Spans the width of the main content region and is center-aligned (`justify-center px-4`) to maintain symmetry with the title and search field.
- **Website Cards:** Styled as pale tertiary panels (`bg-[#FBFBFB] rounded-[13px] border border-[#E6E6E6]`) with atmospheric shadow (`shadow-[0px_6px_10px_-6px_rgba(0,0,0,0.09)]`) and clean hover transitions (`hover:shadow-[0px_10px_20px_-10px_rgba(0,0,0,0.12)] hover:scale-[1.01]`).
- **Inner Preview Thumbnail:** Uses a light gray surface (`bg-[#F7F7F7] border border-[#E6E6E6] rounded-[10px]`) containing the website preview.
- **Controls & Buttons:** Share button is a pill-shaped secondary button (`bg-[#F3F3F3] border border-[#E6E6E6] rounded-[13px] hover:bg-[#EAEAEA] active:scale-[0.97]`).
