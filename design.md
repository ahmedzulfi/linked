...
- **Left Column Panels (Navigation Switcher):**
  - **Design Tab (Conversational AI Wizard):** Chat-like timeline guiding the user step-by-step with interactive forms.
    - **Step 1 (Welcome Kickoff):** Entry welcome screen containing a visual checklist of tasks to complete, displaying the user's name and avatar. All step cards are styled in a premium light editorial theme (`bg-[#FBFBFB]` surfaces with bold `border-2 border-[#2A2A2F]` outlines and solid `shadow-[6px_6px_0px_0px_#2A2A2F]` offset shadows).
    ...
    - **Step 7 (Template Picker):** Selection of theme styles with card-based layouts utilizing matching outline styles and pale blue selected highlights.
