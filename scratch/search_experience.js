...
const terms = ['experience', 'Experience', 'Stripe', 'Figma', 'Worked', 'worked', 'Brands', 'brands'];
terms.forEach(term => {
  let idx = content.indexOf(term);
  if (idx !== -1) {
    console.log(`=== Found "${term}" at index ${idx} ===`);
    console.log(content.substring(Math.max(0, idx - 100), Math.min(content.length, idx + term.length + 100)));
  } else {
